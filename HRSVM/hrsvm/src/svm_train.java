...
public class svm_train {
	...
	private void do_cross_validation()
	{
		...
		svm.svm_cross_validation(prob,param,nr_fold,target, hyp, log_file_name, selected_features, kl_threshold, balance_factor);
	...
	public void run(String argv[]) throws IOException
	{
		...
		else
		{
			...
			//balance_factor = 0.01;
			model = svm.svm_train(prob,param, hyp, log_file_name, selected_features, kl_threshold, balance_factor);
			//DBP
			svm.svm_save_model(model_file_name,model);
			
			//DBP2: Terminate writing to the transfer log - write coefficients, non-crossvalidated
			svm.write_transfer_log_final(model.sv_indices);
			//DBP2
		}
		long end_time = System.currentTimeMillis();
		System.out.println("Total time milliseconds: " + (end_time - start_time));
	}
	...
	private void parse_command_line(String argv[])
	{
		...
		for(i=0;i<argv.length;i++)
		{
			...
			switch(argv[i-1].charAt(1))
			{
				...
				//Extra-parameters				
				case 'f': 
					input_file_name = data_directory + "/" + argv[i];
					break;
					
				case 'y':
					model_file_name = model_directory + "/" + argv[i];
					break;
				...
				case 'a':
					log_file_name = model_directory + "/" + argv[i];
					break;
	...
	private void read_source_hypothesis() throws IOException
	{
		...
		for(int k=0; k<source_hypothesis_file_name.length;k++)
		{
			...
			{
				if(new File(model_directory + "/" + source_hypothesis_file_name[k]).exists())
				{
					BufferedReader fp = new BufferedReader(new FileReader(model_directory + "/" + source_hypothesis_file_name[k]));
					Vector<Double> vy = new Vector<Double>();
					Vector<svm_node[]> vx = new Vector<svm_node[]>();
					int max_index = 0;
					//DBP
					...
					//DBP2: Give an ID to the source hypothesis
					String rep = model_directory.split("/")[model_directory.split("/").length - 1];
					String id = rep + "/" + source_hypothesis_file_name[k];
					hyp[k].ID = id;
					//DBP2
					hyp[k].l = vy.size();
