...
#read data
number_tasks = 100
dims = [3072, 1000, 500, number_tasks]

for rep in range(3,4):
    os.chdir("C:/Users/Administrator/Desktop/other_methods/den")
    import DEN
    
    os.chdir('C:/Users/Administrator/Desktop/cifar/sources/data/rep'+str(rep))
    
    np.random.seed(1004)
    flags = tf.app.flags
    #DBP changed max_iter to a smaller value
    flags.DEFINE_integer("max_iter", 5000, "Epoch to train")
    ...
    flags.DEFINE_integer("ex_k", number_tasks, "The number of units increased in the expansion processing")
    ...
    for t in range(FLAGS.n_classes):
        f = open('C:/Users/Administrator/Desktop/cifar/DEN/den_'+str(number_tasks)+'_tasks_defaultparams_rep'+str(rep)+'.txt', 'a')
        sys.stdout = f
