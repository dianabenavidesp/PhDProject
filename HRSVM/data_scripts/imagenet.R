...
extract_samples_train_validation_test <- function(percent_training, percent_test, percent_validation)
{
  ...
  for(j in 1:500)
  {
    class_data <- read_class_file(classes[j])
    ...
    for(i in 1:1)
    {
      ...
      #just 100 examples
      if(length(training) > 100){ training <- training[1:100] }
  ...
  for(rep in rep)
  {
    for(i in 1:length(training_files))
    {
      ...
      #{
        ...
        output_file <- paste(sources_dir, "model/rep",rep, "/", gsub("_training.csv", "_training.txt", concept), sep = "")
...
train_forward_backward <- function(number_concepts, percent, rep)
{
  ...
  setwd(model_dir)
  hyp <- list.files(pattern = paste(".*\\_training.csv.model$", sep = ""))
  
  setwd(data_dir)
  data <- list.files(pattern = paste(".*\\_training.csv$", sep = ""))
  ...
  source <- hyp[source_indexes[1]]
  sources <- hyp[source_indexes[2:length(source_indexes)]]
...
write_copy_command <- function(model_dir, sources, target, number_concepts, time, rep)
{
  ...
  for(i in 1:length(training_files))
  {
    if(time != 1 & (!paste(training_files[i], "_training_t", time - 1, ".csv.model", sep = "") %in% sources) & training_files[i] != strsplit(target, "_")[[1]][1])
    {
      command <- paste("copy ", paste(model_dir, "/", training_files[i], "_training_t", time - 1, ".csv.model", sep = "" ), paste(model_dir, "/", training_files[i], "_training_t", time, ".csv.model", sep = "" ))
    ...
    else if(time == 1 & (!paste(training_files[i], "_training.csv.model", sep = "") %in% sources) & training_files[i] != strsplit(target, "_")[[1]][1])
    {
      command <- paste("copy ", paste(model_dir, "/", training_files[i], "_training.csv.model", sep = "" ), paste(model_dir, "/", training_files[i], "_training_t", time, ".csv.model", sep = "" ))
      command <- gsub("/", "\\\\", command)
      write.table(command, paste(getwd(), "/forwardbackward_commands_all_imagenet_rep", rep, ".cmd", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, append = TRUE)
    }
  }
...
write_delete_command <- function(model_dir, test_dir, target, number_concepts, time, rep)
{
  ...
  for(i in 1:length(training_files))
  {
    ...
    {
      #delete model
      command <- paste("del ", paste(model_dir, "/", training_files[i], "_training_t", time - 1, ".csv.model", sep = "" ))
      command <- gsub("/", "\\\\", command)
      write.table(command, paste(getwd(), "/forwardbackward_commands_all_imagenet_rep", rep, ".cmd", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, append = TRUE)
      #delete test file
      command <- paste("del ", paste(test_dir, "/", training_files[i], "_test_t", time - 1, ".out", sep = "" ))
      command <- gsub("/", "\\\\", command)
      write.table(command, paste(getwd(), "/forwardbackward_commands_all_imagenet_rep", rep, ".cmd", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, append = TRUE)
    }
    else
    {
      #delete model
      command <- paste("del ", paste(model_dir, "/", training_files[i], "_training.csv.model", sep = "" ))
      command <- gsub("/", "\\\\", command)
      write.table(command, paste(getwd(), "/forwardbackward_commands_all_imagenet_rep", rep, ".cmd", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, append = TRUE)
      #delete test file
      command <- paste("del ", paste(test_dir, "/", training_files[i], "_test_t0.out", sep = "" ))
      command <- gsub("/", "\\\\", command)
      write.table(command, paste(getwd(), "/forwardbackward_commands_all_imagenet_rep", rep, ".cmd", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, append = TRUE)
    }
  }
...
predict_test_command <- function(sources, model_dir, data_dir, test_dir, number_concepts, time, rep)
{
  ...
  for(i in 1:length(training_files))
  {
    ...
    #if(exists_sample(rep, concept, percent))
    #{
      test_file <- paste(data_dir, "/", training_files[i], "_test.csv", sep = "")
      if(time == 0)
      {
        model_file <- paste(model_dir, "/", training_files[i], "_training.csv.model", sep = "")
      }
      else
      {
        model_file <- paste(model_dir, "/", training_files[i], "_training_t", time, ".csv.model", sep = "")
      }
      
      output_file <- paste(test_dir, "/", training_files[i], "_test_t", time, ".out", sep = "")
      txt_file <- paste(test_dir, "/", training_files[i], "_test_t", time, ".txt", sep = "")
