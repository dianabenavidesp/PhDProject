...
extract_samples_train_test <- function(num_concepts)
{
  ...
  {
    class_data <- read.table(paste("newsgroup", class, ".csv", sep = ""))
    
    for(rep in 1:30)
    {
      instances_indexes <- sample(1:nrow(class_data), number_training, replace = FALSE)
      training <- class_data[instances_indexes,]
      
      instances_indexes_test <- sample((1:nrow(class_data))[-instances_indexes], number_test, replace = FALSE)
      test <- class_data[instances_indexes_test,]
      
      write.table(training, file = paste(getwd(), "/rep", rep, "/newsgroup", class, "_training.csv", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
      write.table(test, file = paste(getwd(), "/rep", rep, "/newsgroup", class, "_test.csv", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
      gc()
    }
  }
}
  ...
  for(rep in 1:30)
  {
    for(i in 0:(number_concepts-1))
    {
      ...
      if(exists_sample(rep, concept, percent))
      {
        ...
        output_file <- paste(sources_dir, "model/rep",rep, "/", "newsgroup", concept, "_training.csv.txt", sep = "")
...
train_forward_backward <- function(number_concepts, percent, rep)
{
  ...
  setwd(model_dir)
  hyp <- list.files(pattern = paste("newsgroup.*\\_training.csv.model$", sep = ""))
  
  setwd(data_dir)
  data <- list.files(pattern = paste("newsgroup.*\\_training.csv$", sep = ""))
  
  #next times
  source_indexes <- sample(1:length(hyp), length(hyp) * 0.5, replace = FALSE)
  source <- hyp[as.numeric(gsub("_training.*", "", gsub("newsgroup", "", hyp))) == source_indexes[1]]
  sources <- hyp[as.numeric(gsub("_training.*", "", gsub("newsgroup", "", hyp))) %in% source_indexes[2:length(source_indexes)]]
...
write_copy_command <- function(model_dir, sources, target, number_concepts, time, rep)
{
  ...
  for(i in 0:((number_concepts)-1))
  {
    if(time != 1 & (!paste("newsgroup", i, "_training_t", time - 1, ".csv.model", sep = "") %in% sources) & paste("newsgroup", i, sep = "") != strsplit(target, "_")[[1]][1])
    {
      command <- paste("copy ", paste(model_dir, "/newsgroup", i, "_training_t", time - 1, ".csv.model", sep = "" ), paste(model_dir, "/newsgroup", i, "_training_t", time, ".csv.model", sep = "" ))
    ...
    else if(time == 1 & (!paste("newsgroup", i, "_training.csv.model", sep = "") %in% sources) & paste("newsgroup", i, sep = "") != strsplit(target, "_")[[1]][1])
    {
      command <- paste("copy ", paste(model_dir, "/newsgroup", i, "_training.csv.model", sep = "" ), paste(model_dir, "/newsgroup", i, "_training_t", time, ".csv.model", sep = "" ))
      command <- gsub("/", "\\\\", command)
      write.table(command, paste(getwd(), "/forwardbackward_commands_all_20newsgroups_rep", rep, ".sh", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE, append = TRUE)
    }
  }
...
predict_test_command <- function(sources, model_dir, data_dir, test_dir, number_concepts, time, rep)
{
  ...
  for(i in 0:(number_concepts)-1)
  {
    ...
    if(exists_sample(rep, concept, percent))
    {
      test_file <- paste(data_dir, "/newsgroup", concept, "_test.csv", sep = "")
      if(time == 0)
      {
        model_file <- paste(model_dir, "/newsgroup", concept, "_training.csv.model", sep = "")
      }
      else
      {
        model_file <- paste(model_dir, "/newsgroup", concept, "_training_t", time, ".csv.model", sep = "")
      }
      
      output_file <- paste(test_dir, "/newsgroup", concept, "_test_t", time, ".out", sep = "")
      txt_file <- paste(test_dir, "/newsgroup", concept, "_test_t", time, ".txt", sep = "")
