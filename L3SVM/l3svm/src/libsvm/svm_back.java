...
class Solver_Back {
	...
	void Solve(int l, QMatrix_Back Q, double[] p_, byte[] y_,
		   double[] alpha_, double Cp, double Cn, double eps, SolutionInfo si, int shrinking, double[] initial_gradients, double[] initial_alpha, svm_model[] invariance_models, double gamma_invariant, double[] invariant_gradients)
	{
		this.l = l;
		this.Q = Q;
		QD = Q.get_QD();
		...
		while(iter < max_iter)
		{
			...
			if(invariance_models != null)
			{
				if(!history[i] || !history[j])
				{
					...
					G[i] -= (gradient_invariant_i) * gamma_invariant;

					G[j] -= (gradient_invariant_j) * gamma_invariant;
...
final class Solver_NU_Back extends Solver_Back
{
		   ...
		   SolutionInfo si, int shrinking)
	{
		...
		super.Solve(l,Q,p,y,alpha,Cp,Cn,eps,si,shrinking, null, null, null, 0, null);
	}
...
public class svm_back {
	...
	private static void solve_c_svc(svm_problem prob, svm_parameter param,
					...
					double Cp, double Cn, double[] initial_gradients, double[] initial_alpha, svm_model[] invariance_models, double gamma_invariant)
	{
		int l = prob.l;
		...
		s.Solve(l, q, minus_ones, y,
			alpha, Cp, Cn, param.eps, si, param.shrinking, initial_gradients, initial_alpha, invariance_models, gamma_invariant, invariant_gradients);
					...
					double[] alpha, Solver_Back.SolutionInfo si)
	{
		...
		s.Solve(l, new SVC_Q_Back(prob,param,y, null, 0), zeros, y,
					...
					double[] alpha, Solver_Back.SolutionInfo si)
	{
		...
		s.Solve(l, new ONE_CLASS_Q_Back(prob,param), zeros, ones,
			alpha, 1.0, 1.0, param.eps, si, param.shrinking, null, null, null, 0, null);
	}
					...
					double[] alpha, Solver_Back.SolutionInfo si)
	{
		...
		s.Solve(2*l, new SVR_Q_Back(prob,param), linear_term, y,
			alpha2, param.C, param.C, param.eps, si, param.shrinking, null, null, null, 0, null);
		...
		double Cp, double Cn, double[] initial_gradients, double[] initial_alpha, svm_model[] invariance_models, double gamma_invariant)
	{
		...
		switch(param.svm_type)
		{
			...
			case svm_parameter.C_SVC:
				solve_c_svc(prob,param,alpha,si,Cp,Cn, initial_gradients, initial_alpha, invariance_models, gamma_invariant);
				break;
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB, double[] initial_gradients, double[] initial_alpha, svm_model[] invariance_models, double gamma_invariant)
	{
		int i;
		int nr_fold = 5;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			else
			{
				...
				svm_model submodel = svm_train(subprob,subparam, initial_gradients, initial_alpha, invariance_models, gamma_invariant);
				for(j=begin;j<end;j++)
				{
	...
	//DBP2 Modified to receive invariance model
	private static double svm_svr_probability(svm_problem prob, svm_parameter param, double[] initial_gradients, double[] initial_alpha, svm_model[] invariance_models, double gamma_invariant)
	{
		int i;
		int nr_fold = 5;
		...
		svm_cross_validation(prob,newparam,nr_fold,ymv, initial_gradients, initial_alpha, invariance_models,gamma_invariant);
		for(i=0;i<prob.l;i++)
		{
	...
	public static svm_model svm_train(svm_problem prob, svm_parameter param, double[] initial_gradients, double[] initial_alpha, svm_model[] invariance_models, double gamma_invariant)
	{
		   ...
		   param.svm_type == svm_parameter.NU_SVR)
		{
			    ...
			    param.svm_type == svm_parameter.NU_SVR))
			{
				...
				model.probA[0] = svm_svr_probability(prob,param, initial_gradients, initial_alpha, invariance_models, gamma_invariant);
			}

			decision_function f = svm_train_one(prob,param,0,0, initial_gradients, initial_alpha, invariance_models, gamma_invariant);
			...
			for(i=0;i<nr_class;i++)
				for(int j=i+1;j<nr_class;j++)
				{
					...
					if(param.probability == 1)
					{
						...
						svm_binary_svc_probability(sub_prob,param,weighted_C[i],weighted_C[j],probAB, initial_gradients, initial_alpha, invariance_models, gamma_invariant);
						probA[p]=probAB[0];
						probB[p]=probAB[1];
					}

					//DBP2 But applicable to non-probability models
					f[p] = svm_train_one(sub_prob,param,weighted_C[i],weighted_C[j], initial_gradients, initial_alpha, invariance_models, gamma_invariant);
					for(k=0;k<ci;k++)
	...
	//DBP2 Modified to receive invariance model
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target, double[] initial_gradients, double[] initial_alpha, svm_model[] invariance_models, double gamma_invariant)
	{
		int i;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			svm_model submodel = svm_train(subprob,param, initial_gradients, initial_alpha, invariance_models, gamma_invariant);
			if(param.probability==1 &&
