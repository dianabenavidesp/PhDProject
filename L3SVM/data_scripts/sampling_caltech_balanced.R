...
classes <- c(178, 23, 252, 31, 72, 145, 181, 192)

#1) CREATE SINGLE SAMPLES
read_all_file <- function(classes)
{
  setwd("/data/caltech256")
  ...
  for(cl in unique(all[,1001]))
  {
    if(cl %in% classes)
    {
      write.table(all[all[,1001] == cl,-1001], file = paste("class_", cl, ".csv"), row.names = FALSE, col.names = FALSE, quote = FALSE, sep = ",")
    }
  }
}
...
{
  setwd("/data/caltech256")
  class_data <- read.csv(paste("class_", class, ".csv", sep = ""))
  class_data
}

extract_samples_train_test <- function(class_name, class, percent_training)
{
  setwd(paste("/data/caltech256/samples/class_", class_name, sep = ""))
...
{
  class_data <- read_group_file(class)
  extract_samples_train_test(class, class_data, 0.10)
}

----------------------------
  #2) CREATE SAMPLES AT THE SOURCE, TRANSFER AND TEST LEVEL
  extract_all_data <- function(classes, percent)
  {
    runs <- (length(classes))
    reps <- 30
    ...
    for(run in 2:runs)
    {
      for(rep in 1:30)
      {
        source_actual <- classes[run]
        target_actual <- ifelse(run < (length(classes)), classes[run + 1], classes[1]) 
        
        rest <- classes[1:(length(classes))]
        #this is not the index but the actual value
        rest <- rest[!rest %in% c(source_actual, target_actual)]
        sources_rest <- sample(rest, ceiling((num_classes-1) / 2), replace = FALSE)
        ...
        {
          extract_source(classes, paste("run_", run, "_source", sep = ""), rep, source_actual, percent)
        }
        
        #target actual
        if(!exists_sample(paste("run_", run, "_target", sep = ""), rep, target_actual, percent))
        {
          extract_source(classes, paste("run_", run, "_target", sep = ""), rep, target_actual, percent)
        }
        ...
        for(source_rest in sources_rest)
        {
          ...
          {
            extract_source(classes, paste("run_", run, "_source", sep = ""), rep, source_rest, percent)
          }
        }
        ...
        for(target_rest in targets_rest)
        {
          ...
          {
            extract_source(classes, paste("run_", run, "_target", sep = ""), rep, target_rest, percent)
          }
        }
      }
    }
...
{
  setwd(paste("/data/caltech256/sources/data/",percent*100,"percent/rep",rep,sep = ""))
  
  file.exists(paste(role, "_class_", class_name,"_vsall_balanced_training.csv",sep = ""))
}

extract_source <- function(classes, role, rep, class_name, percent)
{
  negative_all_training <- data.frame()
  negative_all_test <- data.frame()
  
  setwd("/data/caltech256/samples")
  
  for(class in classes)
  {
    #read training
    training_sample <- read.csv(paste(getwd(), "/class_",class,"/",percent*100,"percent/rep",rep,"/training.csv",sep = ""))
    colnames(training_sample) <- c("")
    
    #read test
    test_sample <- read.csv(paste(getwd(), "/class_",class,"/",percent*100,"percent/rep",rep,"/test.csv",sep = ""))
  ...
  negative_all_training <- negative_all_training[sample(1:nrow(negative_all_training), min(round(nrow(positive_training), 0), nrow(negative_all_training)), replace = FALSE, prob = NULL),]
  negative_all_test <- negative_all_test[sample(1:nrow(negative_all_test), min(round(nrow(positive_training), 0), nrow(negative_all_training)), replace = FALSE, prob = NULL),]
  ...
  setwd(paste("/data/caltech256/sources/data/",percent*100,"percent/rep",rep,sep = ""))
  write.table(sample_training, file = paste(role, "_", "class_",class_name,"_vsall_balanced_training.csv",sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE)
  write.table(sample_test, file = paste(role, "_", "class_",class_name,"_vsall_balanced_test.csv",sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE)
}


-----------------
  
  #3)PREPARE DATA FOR LIBSVM (do not scale here but in libsvm)
  prepare_sources <- function(classes, percent)
  {
    for(rep in 1:30)
    {
      setwd(paste("/data/caltech256/sources/data/",percent*100,"percent/rep",rep,sep = ""))
      ...
      for(run in 1:length(classes))
      {
        for(i in 1:length(classes))
        {
          ...
          if(exists_sample(role, rep, class, percent))
          {
            training <- read.csv(paste(getwd(), "/", role, "_class_", class, "_vsall_balanced_training.csv", sep = ""), sep = " ")
            training <- obtain_libsvm_file(training)
            write.table(training, file = paste(getwd(), "/", role, "_class_", class,"_vsall_balanced_training.csv.prescale", sep = ""), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
            
            test <- read.csv(paste(getwd(), "/", role, "_class_", class, "_vsall_balanced_test.csv", sep = ""), sep = " ")
            test <- obtain_libsvm_file(test)
            write.table(test, file = paste(getwd(), "/", role, "_class_", class,"_vsall_balanced_test.csv.prescale", sep = ""), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
          }
...
write_scale_commands <- function(classes, percent)
{
  ...
  for(rep in 1:30)
  {
    for(run in 1:(length(classes)))
    {
      for(i in 1:length(classes))
      {
        ...
        if(exists_sample(role, rep, class, percent))
        {
          setwd("/data/caltech256/sources/")
          file <- paste(getwd(), "/data/",percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_training.csv.prescale", sep = "")
          output_file <- paste(getwd(), "/data/",percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_training.csv.scale", sep = "")
...
write_train_sources_commands <- function(classes, percent)
{
  ...
  for(rep in 1:30)
  {
    for(run in 1:(length(classes)))
    {
      for(i in 1:length(classes))
      {
        ...
        {
          setwd("/data/caltech256/sources/")
          training_file <- paste(getwd(), "/data/",percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_training.csv.scale", sep = "")
          model_file <- paste(getwd(), "/model/",percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_training.csv.model", sep = "")
          output_file <- paste(getwd(), "/model/",percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_training.csv.txt", sep = "")
...
train_forward_backward <- function(classes, percent)
{
  n_features <- 1000
  ...
  for(run in 1:length(classes))
  {
    for(rep in 1:30)
    {
      ...
      data_dir <- (paste("/data/caltech256/sources/data/", percent * 100, "percent/rep", rep, sep = ""))
      model_dir <- (paste("/data/caltech256/sources/model/", percent * 100, "percent/rep", rep, sep = ""))
      
      times <- (length(classes) - 1) / 2
...
#4) PREDICT COMMANDSs
write_predict_commands_after_transfer <- function(classes, percent)
{
  predict_commands <- data.frame(command = character(), stringsAsFactors = FALSE)
  times <- (length(classes) - 1) / 2
  
  for(run in 0:(length(classes)))
  {
    for(rep in 1:30)
    {
      for(i in 1:length(classes))
      {
        ...
        for(time in 0:times)
        {
          #sources
          role <- paste("run_", run, "_source", sep = "")
          if(exists_model_transfer(role, rep, class, percent, time))
          {
            setwd("/data/caltech256/sources/")
            
            test_file <- paste(getwd(), "/data/",percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_test.csv.scale", sep = "")
            ...
            {
              model_file <- paste(getwd(), "/model/",percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_training_t", time, ".csv.model", sep = "")
            }
            
            setwd("/data/caltech256/test/")
            output_file <- paste(getwd(), "/", percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_test_t", time, ".out", sep = "")
            txt_file <- paste(getwd(), "/", percent*100,"percent/rep",rep, "/", role, "_class_", class, "_vsall_balanced_test_t", time, ".txt", sep = "")
...
exists_model_transfer <- function(role, rep, class_name, percent, time)
{
  setwd(paste("/data/caltech256/sources/model/",percent*100,"percent/rep",rep,sep = ""))
  
  if(time > 0) { file.exists(paste(role, "_class_", class_name, "_vsall_balanced_training_t", time, ".csv.model", sep = "")) & file.size(paste(role, "_class_", class_name, "_vsall_balanced_training_t", time, ".csv.model", sep = "")) > 0 }
  else { file.exists(paste(role, "_class_", class_name, "_vsall_balanced_training.csv.model", sep = "")) & file.size(paste(role, "_class_", class_name, "_vsall_balanced_training.csv.model", sep = "")) > 0  }
}